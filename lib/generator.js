  ...
  try {
    ...
    if (stateManagement !== 'None') {
      ...
      if (stateManagement === 'Redux') {
        ...
        // Update App.jsx/tsx to use Redux state & actions
        const appFile = path.join(targetDir, 'src', `App.${mainExt}`);
        if (fs.existsSync(appFile)) {
          let appContent;
          if (language === 'TypeScript') {
            appContent = `import { useSelector, useDispatch } from 'react-redux';
...
function App() {
  ...
  return (
    <div className="app-container">
      ...
      <p>Built with ${buildTool} & React (Redux)</p>
...
`;
        ...
        // Update App.jsx/tsx to use Zustand state & actions
        const mainExt = language === 'TypeScript' ? 'tsx' : 'jsx';
        const appFile = path.join(targetDir, 'src', `App.${mainExt}`);
        if (fs.existsSync(appFile)) {
          const appContent = `import { useStore } from './store/store';
...
function App() {
  ...
  return (
    <div className="app-container">
      <h1>React Forge App</h1>
      <p>Built with ${buildTool} & React (Zustand)</p>
...
`;
      ...
      // Update App.jsx/tsx to show correct buildTool name when no state management is used
      const mainExt = language === 'TypeScript' ? 'tsx' : 'jsx';
      const appFile = path.join(targetDir, 'src', `App.${mainExt}`);
      if (fs.existsSync(appFile) && stateManagement === 'None') {
        let appContent = await fs.readFile(appFile, 'utf8');
        appContent = appContent.replace('Built with Vite & React', `Built with ${buildTool} & React`);
        await fs.writeFile(appFile, appContent);
      }
    }
